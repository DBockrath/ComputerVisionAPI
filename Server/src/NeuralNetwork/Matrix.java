...
public class Matrix {
    ...
    public Matrix add(Matrix matrix) throws Exception {
        ...
        if ((data.length != matrix.data.length) || (data[0].length != matrix.data[0].length)) throw new Exception("Matrices must have matching size");
        double returnData[][] = new double[data.length][data[0].length];

        IntStream.range(0, data.length).forEach( row ->
            IntStream.range(0, data[0].length).forEach( column -> returnData[row][column] = data[row][column] + matrix.data[row][column]));
    ...
    public double dotProduct(Matrix matrix) throws Exception {
        ...
        if (!this.isVector() || !matrix.isVector()) throw new Exception("Can only Dot Product two Vectors");
        else if (this.flatten().length != matrix.flatten().length) throw new Exception("Both Vectors must have the same size");
    ...
    public String toPackedString() {
        ...
        IntStream.range(0, data.length).forEach( row -> {
            IntStream.range(0, data[0].length).forEach( column ->
                body.append((int)data[row][column]));
            body.append("\n");
        });
    ...
    public String toString(String columnLabel, String rowLabel) {
        ...
        heading.append("   |");
        IntStream.range(0, data[0].length).forEach( x -> heading.append("    C" + x + " "));
        heading.append("\n");
        IntStream.range(0, heading.length()).forEach( x -> body.append("-"));
        body.append("\n");
        ...
        IntStream.range(0, data.length).forEach( row -> {
            ...
            IntStream.range(0, data[0].length).forEach( column -> {
                ...
                IntStream.range(0, 1 - (int)(data[row][column] / 10)).forEach(x -> body.append(" "));

                if (data[row][column] >= 0) {

                    body.append("  " + String.format("%.2f", data[row][column]));
        ...
        return heading.toString() + body.toString();
